function [ROCarea_1 ROCarea_1_stim ROCarea_2 ROCarea_2_stim] = Neuron_Data_ROCarray_bootstrap_odrdistVar_Stim(filename,cWindow,TI,TW,iter,cutoff,phase,invert)
...
try
    if ~isempty(MatData)
        ...
        if fixMean < 10000
            ...
            %             ROCarea_4 = [];
            n=0;
            if  length(MatData.class) >= 19%max_class2 &  length([MatData.class(max_class2).ntr]) > 2
                for t = (cWindow(1)-TW/2)/TI:1:(cWindow(2)-TW/2)/TI
                    ...
                    for m = 1:5
                        max_class = m;
                        for i = 1:length(MatData.class(max_class).ntr)
                            ntr_TS1 = MatData.class(max_class).ntr(i).TS;
                            Cue_onT1 = MatData.class(max_class).ntr(i).Cue_onT;
                            maxcl_TS1 = ntr_TS1(find(ntr_TS1 >= Cue_onT1+(TI*t) & ntr_TS1 <Cue_onT1+TI*t+TW));
                            if MatData.class(max_class).ntr(i).Stim
                    ...
                    for m = 11:14
                        max_class=m;
                        for  i = 1:length(MatData.class(max_class).ntr)
                            ntr_TS2 = MatData.class(max_class).ntr(i).TS;
                            Cue_onT2 = MatData.class(max_class).ntr(i).Cue_onT;
                    ...
                    for m = 6:10
                        opp_class = m;
                        for i = 1:length(MatData.class(opp_class).ntr)
                            Cue_onT_opp1 = MatData.class(opp_class).ntr(i).Cue_onT;
                            ntr_TS_opp1 = MatData.class(opp_class).ntr(i).TS;
                            oppcl_TS1 = ntr_TS_opp1(find(ntr_TS_opp1 >= Cue_onT_opp1+(TI*t) & ntr_TS_opp1 < Cue_onT_opp1+TI*t+TW));
                            if MatData.class(opp_class).ntr(i).Stim
                    ...
                    for m = 16:19
                        opp_class = m;
                        for i = 1:length(MatData.class(opp_class).ntr)
                            ntr_TS_opp2 = MatData.class(opp_class).ntr(i).TS;
                            Cue_onT_opp2 = MatData.class(opp_class).ntr(i).Cue_onT;
                            oppcl_TS2 = ntr_TS_opp2(find(ntr_TS_opp2 >= Cue_onT_opp2+(TI*t) & ntr_TS_opp2 < Cue_onT_opp2+TI*t+TW));
                            if MatData.class(opp_class).ntr(i).Stim% 2.20.2018 opp_class1
                    ...
                    if ntr_stim >2 & ntr_stim2>2 & ntr_stim3>2 & ntr_stim4 >2
                        if ~strcmpi(isReverse,'r')
                            ROCarea_1(n) = arrayROC(maxcl_cueall,oppcl_cueall); %  best  vs opp  white nostim
                            ROCarea_1_stim(n) = arrayROC(maxcl_cueall_stim,oppcl_cueall_stim); % best  vs opp  white stim
                            ROCarea_2(n) = arrayROC(maxcl_cueall2,oppcl_cueall2); %  best  vs opp  blue nostim
                            ROCarea_2_stim(n) = arrayROC(maxcl_cueall2_stim,oppcl_cueall2_stim); % best  vs opp  blue stim
